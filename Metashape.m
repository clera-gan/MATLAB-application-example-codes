...
Nmeta=100;
fileID = fopen('polyhedronLib1.dat','w');
fprintf(fileID,'%d\n',Nmeta);

for imeta=1:1:Nmeta
Nnode=18+randi([-2 2],1,1);

theta=rand(Nnode, 1)*pi;
phi=rand(Nnode, 1)*2.0*pi;
r=0.5;
x= (1.0+(rand-0.5)*0.5)*r*sin(theta).*cos(phi);
y= (1.0+(rand-0.5)*0.5)*r*sin(theta).*sin(phi);
z= (1.0+(rand-0.5)*0.5)*r*cos(theta);

vertices = [x y z];
[C,v]= convhulln(vertices);

irad=rand;
% if (irad<0.5)
%     [Nface,ndim] = size(C);
%     Face=[C(:,1) C(:,2) C(:,3)];
%     fprintf(fileID,'%d %d %d %10.6f\n',imeta,Nnode,Nface,v);
%     fprintf(fileID,' %15.6f %15.6f %15.6f\n',vertices');
%     fprintf(fileID,' %8d %8d %8d\n',Face'); 
%     
%   figure(1)
% trisurf(C, vertices(:,1), vertices(:,2), vertices(:,3),'FaceColor','cyan');
%  axis tight;
%  axis square;   
%  continue;
% else  
...
%whos vertices faces
 for i=1:1 %change here to get more smooth surface
 [vertices, faces] = loopSubdivision(vertices, faces);
 figure(2)   
 trisurf(faces', vertices(1,:), vertices(2,:), vertices(3,:),'FaceColor','yel');
 axis tight;
 axis square; 
 end
...
[k1,av1] = convhull(vertices);
Nnode = size(vertices,1);
Nface  = size(faces,1);
faces=[faces(:,1) faces(:,2) faces(:,3)];
fprintf(fileID,'%d %d %d %10.6f\n',imeta,Nnode,Nface,av1);
fprintf(fileID,' %15.6f %15.6f %15.6f\n',vertices');
fprintf(fileID,' %8d %8d %8d\n',faces');
% end
end
